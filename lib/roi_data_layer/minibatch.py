...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        gt_segmentations = np.empty((len(gt_inds), im_blob.shape[1], im_blob.shape[2]), dtype=np.uint8)
        h = roidb[0]['height']
        w = roidb[0]['width']
        ...
        for idx in gt_inds:
            rle_seg = roidb[0]['masks'][idx]
            mask_binary = annToMask(rle_seg, h, w)
            if roidb[0]['flipped']:
                mask_binary = mask_binary[:, ::-1]

            mask_binary = cv2.resize(mask_binary, None, None, fx=im_scales[0], fy=im_scales[0],
                    interpolation=cv2.INTER_LINEAR)

            gt_segmentations[idx] = mask_binary
        
            
        blobs['gt_segmentation'] = gt_segmentations
        ...
        def calc_level(width, height):
            return min(6, max(2, int(5 + np.log2(np.sqrt(width * height) / 224))))
