...
def proposal_target_layer(rpn_rois, gt_boxes, gt_ishard, dontcare_areas, _num_classes):
    ...
    # targets
    labels, rois, bbox_targets, bbox_inside_weights, gt_assignments = _sample_rois(
    ...
    print('num gt: %i, num fg: %i, num bg: %i' % (gt_boxes.shape[0], (labels > 0).sum(), (labels == 0).sum()))
    ...
    def level(roi):
        return calc_level(roi[3] - roi[1], roi[4] - roi[2])   # roi: [0, x0, y0, x1, y1]
    ...
    lens = [len(i) for i in leveled_idxs]
    print(lens)
    lens_np = np.zeros(len(lens) + 1, dtype=np.int32)
    lens_np[1:] = np.array([0] + np.cumsum(lens))

    leveled_idxs = np.array(sum(leveled_idxs, []), dtype=np.int32)

    labels = labels[leveled_idxs]
    gt_assignments = np.array(gt_assignments[leveled_idxs], dtype=np.int32)
    bbox_targets = bbox_targets[leveled_idxs]
    bbox_inside_weights = bbox_inside_weights[leveled_idxs]
    bbox_outside_weights = bbox_outside_weights[leveled_idxs]

    return leveled_rois[0], leveled_rois[1], leveled_rois[2], leveled_rois[3], leveled_rois[4],\
            labels, bbox_targets, bbox_inside_weights, bbox_outside_weights, rois, lens_np, gt_assignments
...
def _sample_rois(all_rois, gt_boxes, gt_ishard, dontcare_areas, fg_rois_per_image, rois_per_image, num_classes):
    ...
    return labels, rois, bbox_targets, bbox_inside_weights, gt_assignment[keep_inds]
