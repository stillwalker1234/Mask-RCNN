...
from ..roi_pooling_layer.roi_align import roi_align, roi_align_max_pooled
...
class Network(object):
    ...
    def fpn_roi_pool(self, input, pooled_height, pooled_width, name):
        ...
        if cfg.USE_ROI_ALIGN:
            roi_pool = roi_align_max_pooled
        else:
            roi_pool = roi_pool_op.roi_pool

        with tf.variable_scope(name) as scope:
            P_range = range(self.fpn_start, self.fpn_end+1)
            strides = [float(2**i) for i in P_range]

            roi_values = []
            for i in P_range:
                roi_values.append(
                    roi_pool(
                        input[i-self.fpn_start], 
                        input[len(P_range)]['fpn_rois'][i-self.fpn_start],
                        pooled_height,
                        pooled_width,
                        1.0 / strides[i-self.fpn_start],
                        name=('roi_pool_P%i' % (i)))[0])

            return tf.concat(axis=0, values=roi_values, name='roi_pool_concat')
    ...
    def proposal_layer(self, input, _feat_strides, anchor_sizes, cfg_key, name):
        ...
        with tf.variable_scope(name) as scope:
            rpn_rois_P2, rpn_rois_P3, rpn_rois_P4, rpn_rois_P5, rpn_rois_P6, rpn_rois, lens, labels = tf.py_func(proposal_layer_py,\
                                                        [input[0], input[1],\
                                                         input[2], input[3],\
                                                         input[4], input[5],\
                                                         input[6], input[7],\
                                                         input[8], input[9],\
                                                         input[10], cfg_key, _feat_strides, anchor_sizes],\
                                                         [tf.float32, tf.float32, tf.float32, tf.float32, tf.float32,tf.float32, tf.int32, tf.int32]);
            ...
            return {
                    #'fpn_rois': [rpn_rois_P2, rpn_rois_P3, rpn_rois_P4, rpn_rois_P5],
                    'fpn_rois': [rpn_rois_P2, rpn_rois_P3, rpn_rois_P4, rpn_rois_P5, rpn_rois_P6],
    ...
    def proposal_target_layer(self, input, classes, name):
        if isinstance(input[0], tuple):
            input[0] = input[0][0]
        with tf.variable_scope(name) as scope:
            # inputs: 'rpn_rois','gt_boxes', 'gt_ishard', 'dontcare_areas', 'masks
            rois_P2,rois_P3,rois_P4,rois_P5,rois_P6, \
            labels,bbox_targets,bbox_inside_weights,bbox_outside_weights, \
            rois,lens,level_idxs \
            ...
            return { 
                # 'fpn_rois': [rois_P2, rois_P3, rois_P4, rois_P5], 
                'fpn_rois': [rois_P2, rois_P3, rois_P4, rois_P5, rois_P6], 
                'labels': tf.convert_to_tensor(tf.cast(labels,tf.int32), name='labels'), 
                'bbox_targets': tf.convert_to_tensor(bbox_targets, name='bbox_targets'),
                'bbox_inside_weights': tf.convert_to_tensor(bbox_inside_weights, name='bbox_inside_weights'),
                'bbox_outside_weights': tf.convert_to_tensor(bbox_outside_weights, name='bbox_outside_weights'),
                'rois': rois,
                'lens': tf.convert_to_tensor(lens, name='lens'),
                'level_idxs': tf.convert_to_tensor(level_idxs)}
