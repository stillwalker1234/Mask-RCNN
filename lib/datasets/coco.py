...
class coco(imdb):
    def __init__(self, image_set, year):
        ...
        self._view_map = {
            'minival2014' : '2014',          # 5k val2014 subset
            'valminusminival2014' : '2014',  # val2014 \setminus minival2014
            'trainplus35kval2014' : '2014',            # train2014 + valminusminival2014
            'val2014': '2014'
        }
    ...
    def image_path_at(self, i):
        ...
        """
        return self._image_path_from_index(i)
    ...
    def _image_path_from_index(self, index):
        ...
        file_name = self.roidb[index]['file_name']
    ...
    def get_json_str(self, idx, labels, dets, mask_pred):
        img_idx = self._image_index[idx]
        ...
        return [
            {
                'image_id': img_idx,
                'category_id': self._class_to_coco_cat_id[self.classes[labels[i]]],
                'bbox': to_xywh(*dets[i, :4]),
                'score': dets[i, -1],
                'is_crowd': 0,
                'segmentation': COCOmask.encode(np.asfortranarray(mask_pred[i])) if mask_pred else []
            }
            for i in range(labels.shape[0]) if labels[i] != 0
        ]

    def write_res_json(self, results_list, output_dir, evaluate=False, eval_metrics=['bbox', 'segm']):
        res_file = osp.join(
            output_dir, 
        ...
        if evaluate:
            if 'bbox' in eval_metrics:
                print("bbox eval...")
                coco_dt = self._COCO.loadRes(res_file)
                coco_eval = COCOeval(self._COCO, coco_dt, iouType='bbox')
                coco_eval.evaluate()
                coco_eval.accumulate()
                coco_eval.summarize()
            
            if 'segm' in eval_metrics:
                print("segmetation eval...")
                coco_dt = self._COCO.loadRes(res_file)
                coco_eval = COCOeval(self._COCO, coco_dt, iouType='segm')
                coco_eval.evaluate()
                coco_eval.accumulate()
                coco_eval.summarize()
