...
class SolverWrapper(object):
    ...
    def snapshot(self, sess, iter):
        ...
        if cfg.TRAIN.BBOX_REG and net.layers.has_key('bbox_pred') and cfg.TRAIN.BBOX_NORMALIZE_TARGETS:
            ...
            sess.run(weights.assign(
                orig_0 * np.tile(self.bbox_stds, (weights_shape[0], 1))))
    ...
    def build_image_summary(self):
        ...
        :return:
        """
        log_image_data = tf.placeholder(tf.uint8, [None, None, None, None])
        ...
        log_image = gen_logging_ops._image_summary(
            log_image_name, log_image_data, max_images=50)
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        tf.summary.scalar('mask_loss', loss_mask)
        ...
        trainable_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, 'res3_5')
        trainable_vars += tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, 'Top-Down')
        trainable_vars += tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, 'RPN')
        trainable_vars += tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, 'Fast-RCNN')
        trainable_vars += tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, 'Mask')
        train_op = opt.minimize(
            loss, global_step=global_step, var_list=trainable_vars)
        ...
        res_fetches = [self.net.get_output('cls_prob'),  # FRCNN class prob
                       ...
                       self.net.get_output('mask_rois'),
                       self.net.get_output('mask_prob'),
                       self.net.get_output('labels_map')]  # RPN rgs output

        if restore_iter != 0:
            lr_factor = int(restore_iter / cfg.TRAIN.STEPSIZE)
            sess.run(tf.assign(lr, lr.eval() * (cfg.TRAIN.GAMMA**lr_factor)))
            print("lr set to %f" % lr.eval())
        ...
        for iter in range(restore_iter, max_iters):
            ...
            feed_dict = {
                ...
                self.net.masks: blobs['gt_segmentation'],
            ...
            fetch_list += res_fetches if (
                iter) % cfg.TRAIN.LOG_IMAGE_ITERS == 0 else []

            if (iter) % cfg.TRAIN.LOG_IMAGE_ITERS == 0:
                rpn_loss_cls_value, rpn_loss_box_value, loss_cls_value, loss_box_value, \
                    summary_str, _, \
                    cls_prob, bbox_pred, rois, mask_rois, mask_pred, labels_map = sess.run(
                        fetches=fetch_list, feed_dict=feed_dict)
            else:
                rpn_loss_cls_value, rpn_loss_box_value, loss_cls_value, loss_box_value, \
            ...
            if (iter) % cfg.TRAIN.LOG_IMAGE_ITERS == 0:
                ...
                ori_im = _draw_gt_to_image(
                    ori_im, blobs['gt_boxes'], blobs['gt_ishard'])
                ori_im = _draw_dontcare_to_image(
                    ori_im, blobs['dontcare_areas'])
                ...
                # transform rois using bbox predictions to refined_rois
                boxes, scores, mask_rois = _process_boxes_scores(
                    cls_prob, bbox_pred, mask_rois, rois, blobs['im_info'][0][2], ori_im.shape)

                # do nms to result
                res = nms_wrapper(scores, boxes, labels_map, mask_pred, mask_rois,
                                  threshold=0.5, cls_names=self.imdb.get_class_names())

                # create final image
                image = cv2.cvtColor(_draw_boxes_to_image(
                    ori_im, res), cv2.COLOR_BGR2RGB)
                ...
                log_image_summary_op = \
                    sess.run(log_image,
                             feed_dict={log_image_name: log_image_name_str,
                                        log_image_data: image[np.newaxis, :, :, :]})
                self.writer.add_summary(
                    log_image_summary_op, global_step=global_step.eval())

                # gt mask rois
                """
                gt_masks = (gt_masks * 255.0).astype('uint8')
                log_image_summary_op = sess.run(log_image, feed_dict={log_image_name: log_image_name_str, log_image_data: gt_masks})
...
def _process_boxes_scores(cls_prob, bbox_pred, maskboxes, rois, im_scale, im_shape):
    """
...
def _draw_boxes_to_image(im, res):
    ...
    for ind, r in enumerate(res):
        ...
        dets = r['dets']
        masks = r['mask']
        mask_rois = r['mask_rois']

        for i in range(0, dets.shape[0]):
            score = dets[i, :][4]
            x1, y1, x2, y2 = [int(item) for item in dets[i, :][:4]]

            color = colors[random.randint(0, len(colors)-1)]

            x1_m, y1_m, x2_m, y2_m = [int(j) for j in mask_rois[i]]
            
            print(x1_m, y1_m, x2_m, y2_m)
            h, w = np.abs(y1_m - y2_m), np.abs(x2_m - x1_m)

            try:
                mask = (sigmoid(cv2.resize(masks[i], (w, h))) > 0.5).astype(np.int32)
                mask = np.tile(np.expand_dims(mask, -1), (1,1,3))
                mask = np.clip(((mask * color) / 2) + image[y1_m:y2_m, x1_m:x2_m].astype(np.int32), 0, 254)
                image[y1_m:y2_m, x1_m:x2_m] = mask.astype(np.uint8)
            except:
                pass

            cv2.rectangle(image, (x1, y1), (x2, y2),
                          colors[ind % len(colors)], 2)
            text = '{:s} {:.2f}'.format(r['class'], score)
            cv2.putText(image, text, (x1, y1), font, 0.6,
                        colors[ind % len(colors)], 1)
            cnt = (cnt + 1)
    return image
...
def train_net(network, imdb, roidb, output_dir, log_dir, pretrained_model=None, max_iters=40000, restore=False):
    ...
    with tf.Session(config=config) as sess:
        sw = SolverWrapper(sess, network, imdb, roidb, output_dir,
                           logdir=log_dir, pretrained_model=pretrained_model)
        print 'Solving...'
